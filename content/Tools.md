---
title: Tools
draft: false
tags:
---
## My equipment

- MacBook Pro 13 M1 (personal PC)
- Apple Magic Keyboard
- Apple Trackpad
- Monitor Gigabite MU32
- Mac Mini M2 (working station for CI/CD)
- MacBook Pro 13 M3 (working PC for developing)

## Productivity

- [Magnet](https://magnet.crowdcafe.com): easily organize windows

## Image processing

- [Shottr](http://shottr.cc/): screenshot capture and blur text
- [Picgo](https://picgo.github.io/PicGo-Doc/en/guide/): image uploader
- [ImageMagick](https://imagemagick.org/): command-line tool to edit and convert images

## Media resources

- [ClipGrab](https://clipgrab.org): online YouTube downloader; can be downloaded as video or audio file

## 📨 Mail

- Spark: smart email management on macOS and iOS. Terrible search though.
- Apple Mail: simple email management

## 📝 Note-taking

- [TextMate](https://macromates.com): plain-text knowledge management.

## ✅ To-do/GTD

- [Things3](https://culturedcode.com/things/): Best to-do app 

## 📆 Calendar

- Apple Calendar

## 📖 Reading/Audiobook

- Apple Books: iOS book reader
- Apple Podcasts
- Litres

## 🔐 Password Management

- Apple Passwords

## 💡 Blogging

- Quarto: blogging solution for my blog
- Quartz: site generator for my digital garden
- [ox-hugo](https://ox-hugo.scripter.co/): convert org-mode notes to hugo-compatible markdown files
- Netlify: website deployment
- Cloudflare: domain provider and website deployment
- Spaceship: domain provider, one of the cheapest in the market

## 🪷 Meditation

- Endel
- Fitness+

## 🔧 Miscs

- [IT Tools](https://it-tools.tech/): handy tools for developers, including various converter
- [Allinone.tools](https://allinone.tools/): convert files

## Pixel art tools

- Collection: [https://github.com/collections/pixel-art-tools](https://github.com/collections/pixel-art-tools)
- Web App: [https://www.piskelapp.com/p/create/sprite](https://www.piskelapp.com/p/create/sprite)